...
def _ffmpeg_path() -> str:
    ...
    if found:
        return found
    raise RuntimeError(
        "ffmpeg not found on PATH. Install it (e.g. https://ffmpeg.org/download.html) "
        "and make sure ffmpeg.exe's folder is added to your PATH environment variable."
    )
...
async def summarize_video(video_path: str) -> str:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        async with httpx.AsyncClient(timeout=120) as client:
            resp = await client.post(
                OLLAMA_URL,
                json={"model": TEXT_MODEL, "messages": [{"role": "user", "content": prompt}], "stream": False},
            )
            resp.raise_for_status()
            return resp.json()["message"]["content"]